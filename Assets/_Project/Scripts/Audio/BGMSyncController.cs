    ...
    public class BGMSyncController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Time.unscaledTime < m_SuspendCorrectionUntilSec)
            {
                ...
                {
                    Debug.Log(
                        $"[BGMSyncController] 補正停止中 reason=focusRecovery expected={expectedMs}ms actual={actualMs}ms drift={driftMs:F1}ms");
        ...
        private void TryReserveSeekCorrectionStage2(int expectedMs, int actualMs, float driftMs, int remainingMsToNextBoundary, int serverTargetAbsoluteBeat)
        {
            ...
            Debug.Log(
                $"[BGMSyncController] Seek予約 serverTargetAbs={m_PendingServerTargetAbsoluteBeat} " +
        ...
        private void OnApplicationFocus(bool hasFocus)
        {
            ...
            {
                Debug.Log(
                    $"[BGMSyncController] 補正停止開始 reason=focusRecovery until={m_SuspendCorrectionUntilSec:F3}");
            }
        }
